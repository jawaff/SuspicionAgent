 ...
 * 
 */
public class WaffleBot extends Bot
{
    Random r = new Random();
    ...
    public WaffleBot(String playerName, String guestName, int numStartingGems, String gemLocations, String[] playerNames, String[] guestNames)
    {
    ...
    public String getPlayerActions(String d1, String d2, String card1, String card2, String board) throws Suspicion.BadActionException
    {
        ...
        for(String cardAction: card.split(":")) // just go ahead and do them in this order
        {
            if(cardAction.startsWith("move")) 
            {
                ...
                piece = pieces.get(guest);
                // We choose a random character, but this selects the best move for our character.
                int[] selectedMove = getMoveToBestGem();
                actions += ":move," + guest + "," + selectedMove[0] + "," + selectedMove[1];
            }
            ...
            else if(cardAction.startsWith("get")) 
            {
                ...
                {
                    gemToGrab = this.board.rooms[me.row][me.col].availableGems[bestGemIndexForRoom(me.row, me.col)];
    ...
    /**
     * Favors gems that we have the least of and picks a random gem if all available gem counts are equal.
     */
    private int bestGemIndexForRoom(int row, int col)
    {
        int minGemCount = countGems(this.board.rooms[row][col].availableGems[0]);
        int bestGemIndex = 0;
        boolean gemCountsEqual = true;
        for (int curGemIndex = 1; curGemIndex < this.board.rooms[row][col].availableGems.length; curGemIndex++)
        {
            int gemCount = countGems(this.board.rooms[row][col].availableGems[curGemIndex]);
            if (gemCount < minGemCount)
            {
                minGemCount = gemCount;
                bestGemIndex = curGemIndex;
            }
            if (gemCount != minGemCount) {
                gemCountsEqual = false;
            }
        }
        ...
        {
            // Grabs a random gem.
            bestGemIndex = r.nextInt(this.board.rooms[row][col].availableGems.length);
        }
        return bestGemIndex;
    }

    /**
     * Determines the best move for our character based on the gem that we have the least of.
     */
    private int[] getMoveToBestGem()
    {
        String bestGem = gemWithLeastCount();
        // We look for rooms with the best gem for us even though we might not have selected our player's character and we choose a random room from that list.
        List<int[]> roomsWithGem = new ArrayList<int[]>();
        for (int row = 0; row < 3; row++)
        {
            for (int col = 0; col < 4; col++)
            {
                if (Arrays.stream(this.board.rooms[row][col].availableGems).anyMatch(bestGem::equals))
                {
                    roomsWithGem.add(new int[] { row, col });
                }
            }
        }
        return roomsWithGem.get(r.nextInt(roomsWithGem.size()));
    }

    private String gemWithLeastCount()
    {
        int leastGemCount = gemCounts[Suspicion.RED];
        int leastGemId = Suspicion.RED;
        boolean gemCountsEqual = true;
        for (int i = 1; i < gemCounts.length; i++)
        {
            if (gemCounts[i] < leastGemCount) {
                leastGemCount = gemCounts[i];
                leastGemId = i;
            }
            if (gemCounts[i] != leastGemCount) {
                gemCountsEqual = false;
        ...
        // If all gem counts are equal, pick a random gem.
        if (gemCountsEqual) {
            leastGemId = r.nextInt(gemCounts.length);
        }

        if (leastGemId == Suspicion.RED) {
            return "red";
        } else if (leastGemId == Suspicion.GREEN) {
            return "green";
        } else {
            return "yellow";
        }
